...
import org.training.facades.registration.MyCustomerFacade;
...
import org.training.storefront.form.validation.MyPhoneValidator;
import org.training.storefront.form.validation.MyProfileValidator;
...
public class AccountPageController extends AbstractSearchPageController
{
	...
	private static final String REDIRECT_TO_UPDATE_PHONE = REDIRECT_PREFIX + "/my-account/update-phone";
	...
	private MyCustomerFacade customerFacade;
	...
	@Resource(name = "myProfileValidator")
	private MyProfileValidator myProfileValidator;

	@Resource(name = "myPhoneValidator")
	private MyPhoneValidator myPhoneValidator;
	...
	/**
	 * @return the myPhoneValidator
	 */
	public MyPhoneValidator getMyPhoneValidator()
	{
		return myPhoneValidator;
	}
	...
	public String editProfile(final Model model) throws CMSItemNotFoundException
	{
		...
		final CustomerData customerData = customerFacade.getCurrentCustomer();
		final MyUpdateProfileForm updateProfileForm = new MyUpdateProfileForm();
		...
		updateProfileForm.setDateOfBirth(customerData.getDateOfBirth());
		updateProfileForm.setMobileNumber(customerData.getPhoneNumber());
		updateProfileForm.setAge(customerData.getAge());
	...
	@RequireHardLogIn
	public String updateProfile(@ModelAttribute("updateProfileForm") final MyUpdateProfileForm updateProfileForm,
			final BindingResult bindingResult, final Model model, final RedirectAttributes redirectAttributes)
			throws CMSItemNotFoundException
	{
		getMyProfileValidator().validate(updateProfileForm, bindingResult);
		...
		customerData.setPhoneNumber(updateProfileForm.getMobileNumber());
		customerData.setDateOfBirth(updateProfileForm.getDateOfBirth());
	...
	/**
	 * @return the myProfileValidator
	 */
	public MyProfileValidator getMyProfileValidator()
	{
		return myProfileValidator;
	}
	...
	@RequireHardLogIn
	public String editPhone(final Model model) throws CMSItemNotFoundException
	{
		model.addAttribute(TITLE_DATA_ATTR, userFacade.getTitles());

		final CustomerData customerData = customerFacade.getCurrentCustomer();
		final MyUpdatePhoneForm updatePhoneForm = new MyUpdatePhoneForm();

		updatePhoneForm.setMobileNumber(customerData.getPhoneNumber());

		model.addAttribute("updatePhoneForm", updatePhoneForm);

		storeCmsPageInModel(model, getContentPageForLabelOrId("update-phone"));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId("update-phone"));

		model.addAttribute(BREADCRUMBS_ATTR, accountBreadcrumbBuilder.getBreadcrumbs("Phone"));
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		return getViewForPage(model);
	}
	...
	@RequireHardLogIn
	public String updatePhone(@ModelAttribute("updatePhoneForm") final MyUpdatePhoneForm updatePhoneForm,
			final BindingResult bindingResult, final Model model, final RedirectAttributes redirectAttributes)
			throws CMSItemNotFoundException
	{

		myPhoneValidator.validate(updatePhoneForm, bindingResult);

		String returnAction = REDIRECT_TO_UPDATE_PHONE;
		final CustomerData currentCustomerData = customerFacade.getCurrentCustomer();
		final CustomerData customerData = new CustomerData();
		customerData.setPhoneNumber(updatePhoneForm.getMobileNumber());
		customerData.setUid(currentCustomerData.getUid());
		customerData.setDisplayUid(currentCustomerData.getDisplayUid());
		customerData.setTitleCode(currentCustomerData.getTitleCode());
		customerData.setName(currentCustomerData.getName());
		model.addAttribute(TITLE_DATA_ATTR, userFacade.getTitles());
		...
		if (bindingResult.hasErrors())
		{
			returnAction = setErrorMessagesAndCMSPage(model, "update-phone");
		}
		else

		{
			try
			{
				customerFacade.updatePhone(customerData);
				GlobalMessages.addFlashMessage(redirectAttributes, GlobalMessages.CONF_MESSAGES_HOLDER,
						"text.account.profile.confirmationUpdated", null);
