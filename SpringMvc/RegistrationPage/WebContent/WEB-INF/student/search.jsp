...
<html>
	...
	<body>
		<div align="center">
		<font style="color: deeppink; font-size: xx-large;"><h1> Student Search Page</h1></font>
		...
		<label style="color: maroon; font-size: x-large;">Search by FirstName or LastName or Email</label><br>
			<form action="${url}">
				<input name="search" value="" style="border: double; padding: 8px 74px;"><br>
			...
			<c:if test="${isSearchResultPage}">
				<c:choose>
					<c:when test="${isSuccess}">
						<font style="color: blue;"><h1> Student Search Results</h1></font>
						<table border="1">
					...
					<c:otherwise>
						<h3><strong>${searchVal}</strong> Student Results not found!!!</h3>
					</c:otherwise>
				</c:choose>
			</c:if>
		</div>
